var widget_chart = {
	...
	init_attr: function(elem) { // initialize all attributes called from widget init function
		...
		if (widget_chart.isIE()) { // hack for unavailability of ln10 function on MS IE
	...
	refresh: function (elem,type,swoffset) { // main function for generation of all HTML code and dynamics for graph called whenever thigs change (e.g. data update, shift, scale, ...)
		...
		tstart.setTime(tstart.getTime() - tstart.dst()*60*1000); // correct daytime savings
		tend.setTime(tend.getTime() - (days_end*24*60*60*1000));
		tend.setTime(tend.getTime() - tend.dst()*60*1000); // correct daytime savings
		...
		data.textHeight = widget_chart.getTextSizePixels(svg_old,data.ytext,'text axes').height;
